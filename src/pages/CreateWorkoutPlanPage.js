...
import {
  loadUser, createWorkoutPlan, updateWorkoutPlan,
...
export default class CreateWorkoutPlanPage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      monthly: false,
    ...
    if (!this.state.user) {
      try {
        ...
        if (this.props.planKey) {
          const plan = user.workoutPlans[this.props.planKey];
          this.setState({
            user,
            plan,
            name: plan.name,
            ...
            exercises: String(plan.exercises),
  ...
  createPlan = async () => {
    try {
      ...
      if (!this.state.exercises) {
        Alert.alert('Please enter the number of exercises in your plan.');
        return;
      }
      ...
      <KeyboardAvoidingView behavior="padding" style={styles.formContainer}>
        <ScrollView contentContainerStyle={styles.center} style={{ width: '100%' }}>
          ...
          <TextField
            icon="info"
            multiline
            placeholder="Plan description (type, exercises, etc.)"
            onChange={(description) => this.setState({ description })}
            value={this.state.description}
          />
          <TextField
            icon="hashtag"
            placeholder="Unique exercises in plan"
            keyboard="number-pad"
            onChange={(exercises) => this.setState({ exercises })}
...
CreateWorkoutPlanPage.propTypes = {
  planKey: PropTypes.string,
};

CreateWorkoutPlanPage.defaultProps = {
  planKey: null,
};
