...
export default class PastGroupSessionDetailsPage extends Component {
  ...
  render() {
    ...
    const clientNames = this.props.session.clients
      ? Object.keys(this.props.session.clients).map((key) => this.props.session.clients[key].name).join(', ')
      : 'None';
    ...
    const rating = this.props.session.trainerKey === userId
      ...
      : this.props.session.clients[userId].rating;
    const total = (this.props.session.cost).toFixed(2);
